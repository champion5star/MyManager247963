...
export async function useAddSignatureAndInitial(payload) {
    try {
        const { data } = await API.post(
            ENDPOINTS.SIGNATURE_AND_INITIAL,
            payload
        )
        return data
    } catch (err) {
...
export async function useGetSignatureAndInitial(email) {
    try {
        const { data } = await API.get(
            `${ENDPOINTS.SIGNATURE_AND_INITIAL}?email=${email}`
        )
       return data
    } catch (err) {
        toast.error('ERROR: An error occured! Please try again ')
...
export async function editDocFromRecipient(id,payload,sendEmail,token){
    const {data} = await API.put(ENDPOINTS.EDIT_RECIPIENTS +id+'?token='+token + '?sendEmail='+sendEmail,payload)
    if (data) {
        if(sendEmail=true){
