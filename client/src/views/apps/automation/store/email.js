...
const domain = process.env.REACT_APP_BASE_URL //"http://localhost:5000"
const baseUrl2 = process.env.REACT_APP_BASE_URL //"http://localhost:5000"
...
export const UPLAODE_IMAGE = (data) => {
    ...
    return async (dispatch) => {
        try {
            let response = await axios.post(url, data, { headers: getHeadersForFile() })
            if (response.data && response.status === 200) {
                toast.success(response.data.msg, toastCSS())
