...
export const renameFolder = createAsyncThunk('filemanager/renameFolder', async ({fileId, folderName}, { dispatch, getState }) => {
  const response = await API.post(`/file-manager/rename-folder/${fileId}`, {folderName});
  await dispatch(getFileAndFolders(getState().filemanager.currentPath));
  return response.data;
})

export const deleteFolder = createAsyncThunk('filemanager/deleteFolder', async (fileId, { dispatch, getState }) => {
