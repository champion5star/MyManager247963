...
const Mails = props => {
  ...
  const renderMails = () => {
    if (tickets.filter((ticket) => ticket.status.toLowerCase() === status).length) {
      return tickets.filter((ticket) => ticket.status.toLowerCase() === status).map((ticket, index) => {
        return (
          <MailCard
            ticket={ticket}
            key={index}
        ...
        <div className='app-action'>
          <div className='action-left form-check'>
            <Input
              ...
              checked={selectedTickets.length && selectedTickets.length === tickets.filter((ticket) => ticket.status.toLowerCase() === status).length}
            />
