...
import {
    workingTaskListAction,
    workingTaskListPastDueAction
} from './store/action'
...
const CheckLists = (props) => {
    ...
    const { workingTaskList, workingTaskListPastDue } = useSelector(
        (state) => state.tasks
    )
    ...
    function formatData(data, searchText, sortVal) {
        ...
        for (let task of data) {
            if (activeTab === 'completed') {
                ...
                if (
                    checkListLength === completedTodosLen ||
                    completedTodosLen > checkListLength
                ) {
                    list.push(task)
                }
        ...
        if (sortVal !== '') {
            if (sortVal === 'asc') {
                // accending sort
                list = list.sort((a, b) =>
                    String(a.taskName).localeCompare(b.taskName)
                )
            }
            ...
            if (sortVal === 'desc') {
                // descending sort
                list = list.sort((a, b) =>
                    String(b.taskName).localeCompare(a.taskName)
                )
            }
