...
exports.addRecipientsWithSignatures = asyncHandler(async (req, res) => {
  ...
  const { recipients, properties, docMessage } = req.body;
    ...
    if (sendEmail && sendEmail === "true") {
      ...
      const allRecipients = recipients.map((r) => {
        const emailBody = documentEmailTemplate({
          ...
          message: docMessage ? docMessage.message : "",
        });
      ...
      allRecipients.forEach((r) => {
        this.sendDocumentEmail({
          from: `${senderName} via MyManager <${sender}>`,
          recipient: r.email,
          subject: docMessage ? docMessage.subject : `New document from ${sender}`,
          body: r.emailBody,
        });
      });
    }
